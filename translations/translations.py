...
def generate_pot():
    # Go through files in 'src'
    # (Exclude .godot and only look in *.tscn and *.gd files)
    # Make a dictionary of all entries, which line they appear on,
    # msgctx from parameters
    ...
    #    #    print(f'    File: {entry['file_path']},
        # #	Block: {entry['block_number']}, Line: {entry['line_number']}')
    with open('translations_template.pot', 'w') as file:
        file.write(
            '# LANGUAGE translation for GoZen -\
            Video Editor for the following files:\n')
        ...
        for key, value in results.items():
            file.write('\n')
            file.write(f'#: {value["file_path"].replace(
                "../src", "res:/")}:{value["line_number"]}\n')
            file.write(f'#: node={value["node_type"]}, type = {
                       value["text_type"]}\n')
...
def _scan_tscn_file(a_file_path, a_results, a_files_list):
    ...
    for line_number, line in enumerate(lines, start=1):
        ...
        if line.startswith("text = "):
            ...
            auto_translate = re.search(
                r'auto_translate = ([^"]+)', line).group(1) != 'false'
...
def generate_mo():
    ...
    for po_file in os.listdir(os.path.join(file_path, 'po_files')):
        ...
        mo_file = os.path.join(
            file_path, '../src/translations/', po_file[:-3] + '.mo')
        os.system(f'msgfmt -o {mo_file} {file_path}/po_files/{po_file}')


def create_new_locale():
    # Ask for locale (Use toolbox and check against possible locales
    # to see if correct)
