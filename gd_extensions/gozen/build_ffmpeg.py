...
folder_bin = os.path.join(os.path.dirname(
    os.path.realpath(__file__)), '../../bin/ffmpeg_bin')
folder_source = os.path.join(os.path.dirname(
    os.path.realpath(__file__)), 'ffmpeg')
...
def configure_ffmpeg():
    ...
    if platform.system().lower() == 'linux':
        ...
        config_extra_args = '--cross-prefix=' + \
            cross_prefix + ' --arch=x86_64 --target-os=mingw32'

    os.system(f'{folder_source}/configure --prefix={
              folder_bin} --enable-gpl --enable-shared {config_extra_args}')
