...
def build_x264(
    ...
    host, sysroot = get_host_and_sysroot(platform, arch)
    ...
    build_lib(
        ...
        + ([f"--host={host}", f"--cross-prefix={host}-", f"--sysroot={convert_to_msys2_path(sysroot)}"] if host else []),
        threads=threads,
        env=env,
        use_msys2=CURR_PLATFORM == "windows",
    )
    ...
    build_lib(
        ...
        + (
            [
                ...
                f"-DCMAKE_SYSROOT={convert_to_msys2_path(sysroot)}",
