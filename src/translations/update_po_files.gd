...
func _run() -> void:
	...
	while file_name != "":
		if not dir.current_is_dir() and file_name.get_extension() == "po":
			...
			if !_merge_po_file(po_path, pot_path):
				Print.info("UpdatePOFiles", "Failed!")
				return
		file_name = dir.get_next()

	EditorInterface.get_resource_filesystem().scan()
	Print.info("UpdatePOFiles", "Complete!")


func _merge_po_file(po_path: String, pot_path: String) -> bool:
	var arguments: PackedStringArray = ["--update", "--backup=none", po_path, pot_path]
	var output: PackedStringArray = []

	Print.info("UpdatePOFiles", "Updating: ", po_path.get_file())
