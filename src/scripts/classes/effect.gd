...
@export var custom_ui: Array[EffectUI]
...
func get_value(effect_param: EffectParam, frame_nr: int) -> Variant:
	...
	return Math.simple_interpolate(value_a, value_b, weight)
...
func get_custom_overlay() -> EffectOverlay:
	if not custom_overlay_path.is_empty() and ResourceLoader.exists(custom_overlay_path):
		return (load(custom_overlay_path) as GDScript).new()
	return null
