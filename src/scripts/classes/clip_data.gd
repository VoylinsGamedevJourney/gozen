...
func load_video_frame(a_track: int, a_frame_nr: int) -> void:
	...
	a_frame_nr *= roundi(_get_file_data().framerate / Project.framerate)
	a_frame_nr = clampi(a_frame_nr, 0, _get_file_data().frame_duration)
	...
	# Check if frame is before current one
	if a_frame_nr < _get_file_data().current_frame[a_track]:
	...
	var l_skips: int = _get_file_data().current_frame[a_track] - a_frame_nr

	# Check if frame is over 10 skips ahead
	if l_skips > MAX_FRAME_SKIPS:
		_get_file_data().current_frame[a_track] = a_frame_nr
		if _get_video(a_track).seek_frame(a_frame_nr):
			printerr("Couldn't seek frame!")
		return
