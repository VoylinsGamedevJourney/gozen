...
func _rebuild_structure() -> void:
	var size: int = Project.tracks.size()
	# Loaded clips setup.
	loaded_clips.resize(size)
	loaded_clips.fill(-1)
...
func find_audio(frame: int, track_id: int) -> int:
	...
	return clip_id if Project.files.get_type(file_index) in AUDIO_TYPES else -1
...
func update_view(track_id: int, update: bool) -> void:
	...
	var effects: Array[GoZenEffectVisual] = Project.clips.get_effects(clip_index).video
