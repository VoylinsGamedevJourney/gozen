...
func _on_project_ready() -> void:
	var project_markers: Dictionary[int, String] = Project.get_markers()

	for frame_nr: int in project_markers:
		_on_marker_added(frame_nr, project_markers[frame_nr])


func _on_marker_added(frame_nr: int, marker: String) -> void:
	...
	markers[frame_nr].text = marker
	add_child(markers[frame_nr])


func _on_marker_updated(old_frame_nr: int, new_frame_nr: int, marker: String) -> void:
