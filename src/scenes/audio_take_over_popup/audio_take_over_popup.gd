...
func load_data(id: int, is_file: bool) -> void:
	...
	var item_id: int = 0
	var audio_files: Dictionary[String, int] = FileLogic.get_all_audio_files()
	...
	video_file_label.text = FileLogic.get_file(current_file_id).nickname
	file_a_player.stream = FileLogic.get_file_data(current_file_id).audio


func _on_take_over_audio_button_pressed() -> void:
	if current_file_id != -1: # file
		#FileLogic.apply_audio_take_over(current_file_id, file_b_id, offset_spinbox.value)
		pass # TODO:
	else: # Clip
...
func _on_audio_file_option_button_item_selected(index: int) -> void:
	...
	file_b_player.stream = FileLogic.get_file_data(file_b_id).audio
